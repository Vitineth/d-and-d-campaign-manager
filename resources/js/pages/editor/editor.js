bus.on("enable-campaign", function(){
    bus.emit("fetch-campaign", function(campaign){
        ...
        function savePuzzle(){
            ...
            campaign = JSON.parse(JSON.stringify(campaign).replace(original, id));
        }
        ...
        function loadMonsterModal(id){
            if(campaign.hasOwnProperty("monsters") && campaign.monsters.hasOwnProperty(id)){
        ...
        function loadPuzzleModal(id){
            if(campaign.hasOwnProperty("puzzles") && campaign.puzzles.hasOwnProperty(id)){
                var puzzle = campaign.puzzles[id];
        ...
        function loadSceneModal(id){
            if(campaign.hasOwnProperty("scenes") && campaign.scenes.hasOwnProperty(id)){
                var scene = campaign.scenes[id];
                ...
                for(var i in scene.npcs){
                    ...
                    for(var i in campaign.npcs){
                        select.append($("<option></option>").text(campaign.npcs[i].name).val("n__" + i));
                    }
                    select.val("n__" + i);
                    npcs.append(element);
                }
                ...
                for(var i in scene.puzzles){
                    var element = $('<li><select></select><i class="material-icons">close</i></li>');
                    var select = element.find("select");
                    for(var i in campaign.puzzles){
                        select.append($("<option></option>").text(campaign.puzzles[i].name).val("p__" + i));
                    }
                    select.val("p__" + i);
                ...
                for(var i in scene.navigation){
                    ...
                    var select = element.find("select");
                    for(var j in campaign.encounters){
                        select.append($("<option></option>").text(campaign.encounters[j].name + " (Encounter)").val("e__" + j));
                    }
                    for(var j in campaign.scenes){
                        select.append($("<option></option>").text(campaign.scenes[j].name + " (Scene)").val("s__" + j));
                    }
                    element.find("input").val(i);
                    if(i.indexOf("s__") == 0){
                        select.val(campaign.scenes[i].name + " (Scene)");
                    }
                    if(i.indexOf("e__") == 0){
                        select.val(campaign.encounters[i].name + " (Encounter)");
                    }
        ...
        function loadEncounterModal(id){
            if(campaign.hasOwnProperty("encounters") && campaign.encounters.hasOwnProperty(id)){
                var encounter = campaign.encounters[id];
                ...
                for(var i in encounter.monsters){
                    var element = $('<li><select></select><i class="material-icons">close</i></li>');
                    var select = element.find("select");
                    for(var i in campaign.monsters){
                        select.append($("<option></option>").text(campaign.monsters[i].name).val("m__" + i));
                    }
                    select.val("m__" + i);
                    npcs.append(element);
                }
                ...
                for(var i in encounter.puzzles){
                    var element = $('<li><select></select><i class="material-icons">close</i></li>');
                    var select = element.find("select");
                    for(var j in campaign.puzzles){
                        select.append($("<option></option>").text(campaign.puzzles[j].name).val("p__" + j));
                    }
        ...
        function loadNPCModal(id){
            if(campaign.hasOwnProperty("npcs") && campaign.npcs.hasOwnProperty(id)){
        ...
        function load(){
            $("#campaign-title").text(campaign.name);
            $("#campaign-title-edit").val(campaign.name);

            for(var i in campaign.encounters){
                $("#begin-event").append($("<option></option>").attr("value", "e__" + i).text(campaign.encounters[i].name + " (Encounter)"));
            }
            for(var i in campaign.scenes){
                $("#begin-event").append($("<option></option>").attr("value", "s__" + i).text(campaign.scenes[i].name + " (Scene)"));
            }
            $("#begin-event").val(campaign.begin);
            ...
            for(var i in campaign.monsters){
                ...
                monster.find("#name").text(campaign.monsters[i].name).removeAttr("id");
                monster.find("#challenge").text("Challenge " + campaign.monsters[i].challenge.level + " (" + campaign.monsters[i].challenge.xp + ")").removeAttr("id");
                monster.find("#stat").text(campaign.monsters[i].hp + "HP/" + campaign.monsters[i].ac + "AC").removeAttr("id");
            ...
            for (var i in campaign.scenes){
                ...
                scene.find("#name").text(campaign.scenes[i].name).removeAttr(i);
                scene.find("#npcs").text(campaign.scenes[i].npcs.length + " NPCs").removeAttr("id");
                scene.find("#puzzles").text(campaign.scenes[i].puzzles.length + " Puzzles").removeAttr("id");
            ...
            for (var i in campaign.puzzles){
                ...
                puzzle.find("#name").text(campaign.puzzles[i].name).removeAttr("id");
                puzzle.find("#hints").text(campaign.puzzles[i].hints.length + " hints").removeAttr("id");
            ...
            for (var i in campaign.encounters){
                ...
                encounter.find("#name").text(campaign.encounters[i].name).removeAttr("id");
                encounter.find("#npcs").text(campaign.encounters[i].npcs.length + " NPCs").removeAttr("id");
                encounter.find("#monsters").text(campaign.encounters[i].monsters.length + " Monsters").removeAttr("id");
                encounter.find("#puzzles").text(campaign.encounters[i].puzzles.length + " Puzzles").removeAttr("id");
            ...
            for (var i in campaign.npcs){
                ...
                npc.find("#name").text(campaign.npcs[i].name).removeAttr("id");
                npc.find("#npcs").text(campaign.npcs[i].class).removeAttr("id");
                npc.find("#monsters").text(campaign.npcs[i].race).removeAttr("id");
                npc.find("#puzzles").text(Object.keys(campaign.npcs[i].key_points).length + " Points").removeAttr("id");
            ...
            $(".material-icons#scene-add-npc").click(function(){
                ...
                var select = element.find("select");
                for(var i in campaign.npcs){
                    select.append($("<option></option>").text(campaign.npcs[i].name));
                }
                ins.append(element);
                reloadClose();
            })
            $(".material-icons#scene-add-puzzle").click(function(){
                ...
                var select = element.find("select");
                for(var i in campaign.puzzles){
                    select.append($("<option></option>").text(campaign.puzzles[i].name));
                }
                ins.append(element);
                reloadClose();
            })
            $(".material-icons#scene-add-navigation").click(function(){
                ...
                var select = element.find("select");
                for(var i in campaign.encounters){
                    select.append($("<option></option>").text(campaign.npcs[i].name + " (Encounter)"));
                }
                for(var i in campaign.scenes){
                    select.append($("<option></option>").text(campaign.scenes[i].name + " (Scene)"));
                }
                ins.append(element);
                reloadClose();
            })
            ...
            $(".material-icons#encounter-add-monsters").click(function(){
                ...
                var select = element.find("select");
                for(var i in campaign.monsters){
                    select.append($("<option></option>").text(campaign.monsters[i].name));
                }
                ins.append(element);
                reloadClose();
            })
            $(".material-icons#encounter-add-navigation").click(function(){
                ...
                var select = element.find("select");
                for(var i in campaign.encounters){
                    select.append($("<option></option>").text(campaign.encounters[i].name + " (Encounter)"));
                }
                for(var i in campaign.scenes){
                    select.append($("<option></option>").text(campaign.scenes[i].name + " (Scene)"));
                }
